...
import SignUpPage from './pages/SignUpPage'; // Added SignUpPage
import ForgotPasswordPage from './pages/ForgotPasswordPage'; // Added ForgotPasswordPage
...
const DynamicPageTitle: React.FC<{ appSettings: AppSettings }> = ({ appSettings }) => {
  ...
  useEffect(() => {
    ...
    const navItem = NAV_ITEMS.find(item => {
      ...
      // More robust matching for paths like /students vs /students/someId
    ...
    if (navItem) {
      ...
      // Handle non-nav item routes based on Page enum or custom strings
      if (currentPath.startsWith('/login')) pageName = 'Login';
      ...
      else if (currentPath.startsWith('/signup')) pageName = 'Sign Up';
      else if (currentPath.startsWith('/forgot-password')) pageName = 'Forgot Password';
      // Note: /admin/settings is in NAV_ITEMS, so it should be caught above.
      else if (currentPath === '/') pageName = Page.Dashboard; // Default for root if not caught
    }
...
const App: React.FC = () => {
  ...
  if (!isAdminLoggedIn) {
    return (
      <HashRouter>
        <DynamicPageTitle appSettings={appSettings} />
        <Routes>
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          >
            <Routes>
              ...
              <Route path="/gallery" element={<PhotoGalleryPage photos={photos} addPhoto={addPhoto} updatePhoto={updatePhoto} deletePhoto={deletePhoto} headingTextColor={headingTextColor} />} />
