...
const LoginPage: React.FC<LoginPageProps> = ({ onLoginSuccess, appSettings }) => {
  ...
  const { 
    ...
    collegeLogo, 
    adminUsername = DEFAULT_APP_SETTINGS.adminUsername, 
    adminPassword = DEFAULT_APP_SETTINGS.adminPassword,
    loginPageMainHeading = DEFAULT_APP_SETTINGS.loginPageMainHeading
  } = appSettings;
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    const effectiveUsername = adminUsername; 
      ...
      <div className="w-full max-w-md">
         <div className="text-center mb-8">
            ...
            <p className="text-sky-200 mt-1">
              {collegeAddress} | Tel: <a href={formatTelLink(collegePhone)} className="hover:underline">{collegePhone}</a>
            </p>
        </div>

        <form onSubmit={handleLogin} className="bg-white shadow-2xl rounded-xl p-8 space-y-6">
          <h2 className="text-2xl font-semibold text-center text-slate-700 mb-6">{loginPageMainHeading}</h2>
