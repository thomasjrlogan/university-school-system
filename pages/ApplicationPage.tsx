...
import { StudentApplication, TeacherApplication, UploadedFile, AppSettings, Course } from '../types'; 
...
import { 
    ...
    BookOpenIcon, SparklesIcon, ClockIcon, LightBulbIcon, HomeIcon // Replaced BriefcaseClockIcon with ClockIcon
...
const ApplicationPage: React.FC<ApplicationPageProps> = ({ appSettings, courses }) => {
  ...
  const { appName, collegeLogo, applicationFee, bankName, bankAccountName, bankAccountNumber, bankSwiftCode, mobileMoneyProvider, mobileMoneyNumber } = appSettings;
  ...
  const handleStudentSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSubmissionStatus({ type: 'success', message: 'Student application submitted successfully! We will review your application and contact you soon.', applicationId: newApplicationId, applicationType: 'student' });
  ...
  const handleTeacherSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSubmissionStatus({ type: 'success', message: 'Teacher application submitted successfully! We will review your application and contact you soon.', applicationId: newApplicationId, applicationType: 'teacher' });
  ...
  const handlePrintApplication = useCallback((appId?: string, appType?: ApplicationType) => {
    ...
    const { collegeLogo: currentLogo, appName: currentAppName, collegeAddress: currentAddress, collegePhone: currentPhone } = appSettings;
    ...
    const fee = appSettings.applicationFee;
    if (fee && fee > 0 && (appSettings.bankName || appSettings.mobileMoneyProvider)) {
        ...
        if (appSettings.bankName && appSettings.bankAccountNumber && appSettings.bankAccountName) {
            ...
            paymentInstructionsHtmlForPrint += `<h4 style="font-size: 12pt; font-weight: bold; margin-top:0; margin-bottom: 8px; color: #444;">Bank Deposit / Transfer:</h4>`;
            paymentInstructionsHtmlForPrint += `<p style="font-size: 10pt; margin-bottom: 3px;"><strong>Bank Name:</strong> ${appSettings.bankName}</p>`;
            paymentInstructionsHtmlForPrint += `<p style="font-size: 10pt; margin-bottom: 3px;"><strong>Account Name:</strong> ${appSettings.bankAccountName}</p>`;
            paymentInstructionsHtmlForPrint += `<p style="font-size: 10pt; margin-bottom: 3px;"><strong>Account Number:</strong> ${appSettings.bankAccountNumber}</p>`;
            if (appSettings.bankSwiftCode) paymentInstructionsHtmlForPrint += `<p style="font-size: 10pt; margin-bottom: 3px;"><strong>SWIFT/BIC Code:</strong> ${appSettings.bankSwiftCode}</p>`;
            paymentInstructionsHtmlForPrint += `</div>`;
        }
        if (appSettings.mobileMoneyProvider && appSettings.mobileMoneyNumber) {
            paymentInstructionsHtmlForPrint += `<div style="margin-bottom: 12px; padding: 12px; border: 1px solid #eee; background-color: #fdfdfd; border-radius: 4px;">`;
            paymentInstructionsHtmlForPrint += `<h4 style="font-size: 12pt; font-weight: bold; margin-top:0; margin-bottom: 8px; color: #444;">Mobile Money:</h4>`;
            paymentInstructionsHtmlForPrint += `<p style="font-size: 10pt; margin-bottom: 3px;"><strong>Provider:</strong> ${appSettings.mobileMoneyProvider}</p>`;
            paymentInstructionsHtmlForPrint += `<p style="font-size: 10pt; margin-bottom: 3px;"><strong>Number:</strong> ${appSettings.mobileMoneyNumber}</p>`;
            paymentInstructionsHtmlForPrint += `</div>`;
        }
        paymentInstructionsHtmlForPrint += `<p style="font-size: 9pt; color: #666; margin-top: 15px;">After payment, please allow up to 48 hours for confirmation. You may be contacted if further information is required.</p>`;
        paymentInstructionsHtmlForPrint += `</div>`;
    }
  ...
  const showPaymentInstructions = submissionStatus?.type === 'success' && applicationFee && applicationFee > 0 && (bankName || mobileMoneyProvider);
  ...
  return (
    <div className="min-h-screen bg-slate-100 py-8 px-4 sm:px-6 lg:px-8">
      <header className="text-center mb-10">
        ...
        <p className="text-xl text-slate-600 mt-1">Online Application Portal</p>
      </header>
        ...
        {showPaymentInstructions && submissionStatus?.applicationId && (
            <div id="payment-instructions-section" className="p-4 mb-6 rounded-md border border-sky-300 bg-sky-50 text-slate-700">
                <h3 className="text-lg font-semibold text-sky-700 mb-3 flex items-center">
                    <CurrencyDollarIcon className="h-6 w-6 mr-2"/> Payment Instructions
                </h3>
                ...
                {bankName && bankAccountNumber && bankAccountName && (
                    <div className="mb-4 p-3 border rounded-md bg-white">
                        <h4 className="font-semibold text-sky-600 mb-1 flex items-center"><CreditCardIcon className="h-5 w-5 mr-2"/>Bank Deposit / Transfer:</h4>
                        <p className="text-xs"><strong>Bank Name:</strong> {bankName}</p>
                        <p className="text-xs"><strong>Account Name:</strong> {bankAccountName}</p>
                        <p className="text-xs"><strong>Account Number:</strong> {bankAccountNumber}</p>
                        {bankSwiftCode && <p className="text-xs"><strong>SWIFT/BIC Code:</strong> {bankSwiftCode}</p>}
                    </div>
                )}

                {mobileMoneyProvider && mobileMoneyNumber && (
                     <div className="mb-2 p-3 border rounded-md bg-white">
                        <h4 className="font-semibold text-sky-600 mb-1 flex items-center"><DevicePhoneMobileIcon className="h-5 w-5 mr-2"/>Mobile Money:</h4>
                        <p className="text-xs"><strong>Provider:</strong> {mobileMoneyProvider}</p>
                        <p className="text-xs"><strong>Number:</strong> {mobileMoneyNumber}</p>
                    </div>
                )}
                <p className="text-xs text-slate-500 mt-3 flex items-start">
                    <InformationCircleIcon className="h-4 w-4 mr-1.5 mt-0.5 text-sky-500 flex-shrink-0"/>
                    After payment, please allow up to 48 hours for confirmation. You may be contacted if further information is required.
                </p>
            </div>
        )}
          ...
          <>
            {activeForm === 'student' && (
              <form id="student-application-form" onSubmit={handleStudentSubmit} className="space-y-6">
                <h2 className="text-2xl font-semibold text-slate-700 mb-4">Student Application Form</h2>
            ...
            {activeForm === 'teacher' && (
              <form id="teacher-application-form" onSubmit={handleTeacherSubmit} className="space-y-6">
                <h2 className="text-2xl font-semibold text-slate-700 mb-4">Teacher Application Form</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                   <div>
