...
// APP_NAME, COLLEGE_ADDRESS, COLLEGE_PHONE are now from appSettings
...
export const StudentResultsPage: React.FC<StudentResultsPageProps> = ({ students, courses, enrollments, appSettings }) => {
  ...
  const handlePrint = () => {
    ...
    const logoHtml = appSettings.collegeLogo 
      ? `<img src="${appSettings.collegeLogo}" alt="University Logo" style="height: 60px; margin-bottom: 10px; max-width: 150px; object-fit: contain;" />`
      : `<div style="height: 60px; display: flex; align-items: center; justify-content: center; font-size: 20px; font-weight: bold;">${appSettings.appName.substring(0,3)}</div>`;
    ...
    const headerHtml = `
      <div style="text-align: center; margin-bottom: 20px; font-family: Arial, sans-serif;">
        ${logoHtml}
        <h1 style="font-size: 24px; margin: 0;">${appSettings.appName}</h1>
        <p style="font-size: 12px; margin: 2px 0;">${appSettings.collegeAddress}</p>
        <p style="font-size: 12px; margin: 2px 0;">Tel: ${appSettings.collegePhone}</p>
    ...
    // Remove the student info div from the resultsRef content if it's duplicated from the live view
    const tempDiv = document.createElement('div');
    tempDiv.innerHTML = printContents;
    const studentInfoDivInResults = tempDiv.querySelector('.student-info-for-print-only'); // Add this class to the div in render
    ...
    <div className="min-h-screen bg-slate-100 py-8 px-4 sm:px-6 lg:px-8 print:bg-white">
      <style>{`
        @media print {
          ...
          .student-info-for-print-only { display: none !important; } /* Hide the live view version */
        }
      `}</style>

      <header className="text-center mb-10 no-print">
        {appSettings.collegeLogo ? (
            <img src={appSettings.collegeLogo} alt={`${appSettings.appName} Logo`} className="h-20 w-auto object-contain mx-auto mb-2 rounded-md" />
        ) : (
            <AcademicCapIcon className="h-16 w-16 text-sky-600 mx-auto mb-2" />
        )}
        <h1 className="text-4xl font-bold text-slate-800 tracking-tight">${appSettings.appName}</h1>
        <p className="text-xl text-slate-600 mt-1">Student Results Portal</p>
      </header>

      <div className="max-w-2xl mx-auto">
        <Card className="shadow-xl no-print">
          <div className="p-6">
            <h2 className="text-xl font-semibold text-slate-700 mb-4">Check Your Academic Results</h2>
            <div className="flex items-end gap-3 mb-4">
              <Input
                id="studentIdInput"
                label="Enter Your Student ID, System ID or Email"
                placeholder="e.g., CUC2024001, student-xyz, or name@example.com"
          ...
          <div className="mt-8">
            <Card className="shadow-xl printable-card"> 
              <div className="p-6"> 
                ...
                {/* This div is for live view and will be hidden in print by CSS, its content is added via headerHtml */}
                ...
                <div ref={resultsRef} className="printable-content"> {/* Table content for printing */}
       ...
       <p className="text-center text-xs text-slate-500 mt-10 no-print">&copy; {new Date().getFullYear()} {appSettings.appName}. All rights reserved.</p>
    </div>
  );
};
